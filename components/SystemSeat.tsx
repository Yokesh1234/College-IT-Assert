...
const SystemSeat: React.FC<SystemSeatProps> = ({ system, isSelected, onClick }) => {
  ...
  const getStatusIcon = () => {
    ...
    switch (system.status) {
      case SystemStatus.WORKING: return <i className="fa-solid fa-check text-[10px] text-white"></i>;
      case SystemStatus.PARTIAL: return <i className="fa-solid fa-triangle-exclamation text-[10px] text-white"></i>;
      case SystemStatus.NOT_WORKING: return <i className="fa-solid fa-xmark text-[10px] text-white"></i>;
    }
  };
  ...
  return (
    ...
    >
      ...
      {system.bookings.length > 0 && !isSelected && (
        <div className="absolute -top-1 -right-1 w-4 h-4 bg-blue-600 border border-white rounded-full flex items-center justify-center">
      ...
      {!isSelected && (
        <div className="absolute bottom-full mb-2 hidden group-hover:block z-50 bg-slate-900 text-white text-[10px] p-2 rounded whitespace-nowrap border border-slate-700 shadow-2xl animate-in fade-in slide-in-from-bottom-1">
          ...
          <div>RAM: {system.hardware.ram}</div>
          <div className="mt-1 font-semibold uppercase text-slate-400">{system.status.replace('_', ' ')}</div>
        </div>
      )}
    </div>
  );

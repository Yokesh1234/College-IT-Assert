...
import { System, GridConfig, TimetableEntry, SystemStatus } from '../types';
...
const LabMap: React.FC<LabMapProps> = ({ 
  ...
  const renderTable = (labIndex: number) => {
    ...
    // Extract potential batch code (e.g. "C4" from "TB-C4" or "C4")
    const batchCode = displayName.replace('TB-', '').trim().toUpperCase();

    // Find if there's any class scheduled for this table at the current day and period
    const activeClass = timetables.find(t => 
      t.day === simulatedDay && 
      t.period === simulatedPeriod && 
      t.batch.toUpperCase() === batchCode
    ) || null;

    const periodLabel = `Period ${simulatedPeriod}`;
    ...
    for (let i = 0; i < SYSTEMS_PER_LAB; i++) {
      ...
      if (system) {
        ...
        // 3. BOOKED
        // 4. FREE
        let currentStatus: 'FREE' | 'RUNNING CLASS' | 'BOOKED' | 'UNDER MAINTENANCE' = 'FREE';
        if (system.status === SystemStatus.NOT_WORKING) {
          currentStatus = 'UNDER MAINTENANCE';
        } else if (activeClass) {
          currentStatus = 'RUNNING CLASS';
        } else if (system.bookings && system.bookings.length > 0) {
          // If there's an active booking for today/period
